...
export default function Intro({ t }: { t?: any }) {
  ...
  const services = [
    ...
    },
    {
      ...
      // Rotating play-like triangle
      accent: (
        <motion.div
          animate={{ rotate: [0, 90, 180, 270, 360] }}
          transition={{ duration: 8, repeat: Infinity, ease: "linear" }}
          className="h-0 w-0 border-y-[5px] border-l-[8px] border-y-transparent border-l-[#6C0B1C]/60 ml-0.5"
        />
      ),
    },
    {
  ...
  return (
    <section className="relative overflow-hidden bg-[#FAF8F6] px-6 py-24 md:py-32">
      {/* Subtle Background Blobs for the section */}
      <div className="absolute -left-[10%] top-[20%] h-[400px] w-[400px] rounded-full bg-[#6C0B1C]/5 blur-[100px] pointer-events-none" />
      <div className="absolute -right-[10%] bottom-[10%] h-[300px] w-[300px] rounded-full bg-[#480713]/5 blur-[80px] pointer-events-none" />
      ...
      <div className="mx-auto grid max-w-7xl items-center gap-16 lg:grid-cols-[0.9fr_1.1fr]">
        ...
        <div className="relative hidden h-[550px] w-full lg:block pointer-events-none">
        ...
        >
          <motion.div variants={fadeUp} className="mb-6 inline-flex items-center gap-3 rounded-full border border-[#6C0B1C]/15 bg-white/50 px-4 py-2 text-xs font-bold uppercase tracking-[0.2em] text-[#6C0B1C] shadow-sm backdrop-blur-md">
            <span className="flex h-1.5 w-1.5 rounded-full bg-[#6C0B1C]"></span>
            What We Do
          </motion.div>

          <motion.h2 
            variants={fadeUp} 
            className="max-w-xl font-serif text-4xl font-medium leading-[1.15] tracking-[-0.02em] text-[#1B1B1B] md:text-5xl"
          >
          ...
          <motion.p 
            variants={fadeUp} 
            className="mt-6 max-w-xl text-base leading-relaxed text-[#5E5E5E] md:text-lg"
          >
          ...
          <motion.div variants={fadeUp} className="mt-12 grid gap-4 sm:grid-cols-2">
            {services.map((card) => (
              ...
              >
                <div className="mb-5 flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-white shadow-sm border border-[#6C0B1C]/5 transition-colors group-hover:border-[#6C0B1C]/20">
                  {card.accent}
                </div>
